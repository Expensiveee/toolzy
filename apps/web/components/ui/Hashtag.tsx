...
const hashtag = cva(
  "rounded-full bg-dark-700 hover:bg-dark-700/70 border border-dark-400/40 text-white cursor-pointer px-4 py-2.5 text-sm transition-colors duration-300 ease-in-out font-normal"
);
...
const Hashtag: React.FC<HashtagProps> = ({ className, text, emoji, ...props }) => {
  ...
  return (
    <span {...props} className={combinedClassName}>
      #{text}
      <Image alt={`${emoji} emoji`} src={`/emojis/${emoji}.svg`} height={16} width={16} />
    </span>
  );
};
